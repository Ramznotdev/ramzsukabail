...
function signalStorage({ creds, keys }, lidMapping, logger) {
  ...
  return {
    ...
    // Sender keys are stored as JSON-stringified buffers to match SenderKeyRecord.deserialize expectations
    storeSenderKey: async (keyId, record) => {
      const id = keyId.toString()
      let serialized
      if (typeof record?.serialize === 'function') {
        serialized = JSON.stringify(record.serialize())
      } else if (record instanceof Uint8Array || Buffer.isBuffer(record)) {
        // already serialized binary from Rust bridge — wrap as JSON array for round-trip compatibility
        serialized = JSON.stringify(Array.from(record))
      } else {
        serialized = JSON.stringify(record)
      }
      await keys.set({ 'sender-key': { [id]: Buffer.from(serialized, 'utf-8') } })
    },
