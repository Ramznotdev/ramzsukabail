...
// v2 slot holds the actual rust-bridge serialized SessionRecord bytes
// plain slot holds a compat marker so old JS code sees something
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
  ...
  // ensures a sender key record exists before group session operations
  const ensureSenderKey = async (senderName) => {
    const keyId = senderName.toString()
    const { [keyId]: existing } = await parsedKeys.get('sender-key', [keyId])
    if (!existing) await storage.storeSenderKey(senderName, new Uint8Array(0))
  }
  ...
  return {
    ...
    async decryptMessage({ jid, type, ciphertext }) {
      ...
      const doDecrypt = (c, t) => {
        ...
        throw new Error(`Unknown decrypt type: ${t}`)
      }
      try {
        return await txn(() => doDecrypt(cipher, type), jid)
      } catch (e) {
        if (e?.message?.includes('DuplicatedMessage')) {
          logger?.debug?.({ jid }, '[Signal] Duplicate message ignored — offline replay')
          return null
        }
        throw e
      }
    ...
    async encryptGroupMessage({ group, meId, data }) {
      return txn(async () => {
        ...
        const ciphertext = await new GroupCipher(storage, group, jidToAddr(meId)).encrypt(data)
    ...
    async getSessionInfo(jid) {
      try {
        ...
        const batch = await migrateIndexKey(parsedKeys, 'session')
        const raw = toBuffer(batch[v2Key(addr)]) || toBuffer(batch[addr])
        if (!raw || isOldJson(raw)) return undefined
        const sess = SessionRecord.deserialize(raw)
        return {
          registrationId: 0,
          baseKey: undefined,
          haveOpenSession: sess.haveOpenSession()
        }
    ...
    // batch-migrates ALL PN sessions → LID in one read + one write on connect
    async migrateAllPNSessionsToLID() {
      return txn(async () => {
        ...
        for (const pnUser of pnUserSet) {
          ...
          if (lidUser && typeof lidUser === 'string') pnToLidMap.set(pnUser, lidUser)
        }
        ...
        if (migrated > 0) {
          ...
          logger?.info?.({ migrated, totalPN: pnAddrs.length, mappingsFound: pnToLidMap.size }, '[Signal] Batch-migrated PN sessions to LID on connect')
        }
        return migrated
...
function signalStorage({ creds, keys }, lidMapping, logger) {
  // 5-min LRU cache for PN→LID address resolutions
  ...
  return {
    ...
    storeSession: async (id, session) => {
      const addr = await resolveLID(id)
      const batch = await getIndex()
      batch[v2Key(addr)] = session instanceof Uint8Array || Buffer.isBuffer(session) ? session : session.serialize()
    ...
    storeSenderKey: async (keyId, record) => {
      const id = keyId.toString()
      const bytes = record instanceof Uint8Array ? record : Buffer.isBuffer(record) ? record : record.serialize?.() ?? new Uint8Array(0)
