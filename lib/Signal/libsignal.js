...
// coerce to plain Uint8Array — bridge requires this for encrypt/decrypt inputs
const toU8 = (raw) => {
...
// Reads field 4 (identity key, 33 bytes) from the protobuf envelope
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    return {
        ...
        async processSenderKeyDistributionMessage({ item, authorJid }) {
            ...
            // NOTE: do NOT pre-store a blank SenderKeyRecord before process() —
            // SenderKeyRecord.serialize() returns 0 bytes and the bridge throws on empty records.
            // GroupSessionBuilder.process() handles a null loadSenderKey return correctly.
        ...
        encryptGroupMessage({ group, meId, data }) {
            return txn(async () => {
                ...
                // create() loads existing record or initialises from null — never pass a blank record.
                // encrypt() calls storeSenderKey back after advancing the chain counter — confirmed by test.
        ...
        async migrateAllPNSessionsToLID() {
            // lid-mapping read outside txn — avoids nested key-namespace lock
            const sessionBatch = await migrateIndexKey(parsedKeys, 'session')
...
//
// Session storage uses a dual-key index pattern:
//   v2Key(addr) → actual binary SessionRecord bytes
//   addr        → JSON tombstone { version:'v1', _sessions:{} } for old-code compat
//
// Sender keys are stored flat by key ID — the bridge always passes/receives
// plain Uint8Array (confirmed by test), never a SenderKeyRecord object.
...
function signalStorage({ creds, keys }, lidMapping, logger) {
    ...
    return {
        // loadSession(address: string) → Uint8Array | null
        ...
        // storeSession(address: string, record: SessionRecord) → void
        ...
        // isTrustedIdentity(name, identityKey, direction) → boolean
        isTrustedIdentity: () => true,

        // internal — used by decryptMessage identity change detection
        ...
        saveIdentity: async (id, identityKey) => {
            ...
            if (existing && !match) {
                // wipe session from index before updating identity key
                const batch = await getIndex()
                delete batch[addr]
                delete batch[v2Key(addr)]
                await setIndex(batch)
        ...
        // loadPreKey(id: number) → KeyPair | null
        loadPreKey: async (id) => {
        ...
        // loadSignedPreKey(id: number) → SignedPreKey | null
        // WA only ever has one active signed pre-key — we log a mismatch but return it anyway
        loadSignedPreKey: (id) => {
            const key = creds.signedPreKey
            if (key.keyId !== id) logger?.warn?.({ requested: id, current: key.keyId }, '[Signal] loadSignedPreKey id mismatch')
            return {
                keyId: key.keyId,
                keyPair: {
        ...
        // loadSenderKey(keyId: string) → Uint8Array | null
        // bridge always passes plain Uint8Array to storeSenderKey — confirmed by test
        ...
        // storeSenderKey(keyId: string, record: Uint8Array) → void
        // record is always plain Uint8Array from the bridge — store directly, no .serialize() needed
        // NEVER pre-store a blank SenderKeyRecord — serialize() returns 0 bytes and bridge throws
