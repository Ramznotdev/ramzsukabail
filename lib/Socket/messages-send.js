...
export const makeMessagesSocket = (config) => {
    ...
    const getPrivacyTokens = async (jids) => {
        const result = await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, type: 'get', xmlns: 'privacy' },
            content: [{ tag: 'tokens', attrs: {}, content: jids.map(jid => ({ tag: 'token', attrs: { jid: jidNormalizedUser(jid), type: 'trusted_contact' } })) }]
        })
        ...
        if (tokenList) {
            for (const node of getBinaryNodeChildren(tokenList, 'token')) {
                const rawJid = node.attrs.jid
                const content = node.content
                if (!rawJid || !content) continue
                const storageJid = await resolveTcTokenJid(jidNormalizedUser(rawJid), getLIDForPN)
                const ts = node.attrs.t ? Number(node.attrs.t) : unixTimestampSeconds()
                tokens[storageJid] = { token: Buffer.from(content), timestamp: String(ts) }
            }
        }
        if (Object.keys(tokens).length) {
            const indexWrite = await buildMergedTcTokenIndexWrite(authState.keys, Object.keys(tokens))
            await authState.keys.set({ tctoken: { ...tokens, ...indexWrite } })
        }
        return tokens
    }
