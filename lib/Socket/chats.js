...
export const checkStatusWA = async (phoneNumber) => {
    ...
    const { countryCallingCode: countryCode, nationalNumber } = parsePhoneNumber(formattedNumber)
    const { mobileRegisterExists, getBanDetails } = await import('./registration.js')
    const { useMultiFileAuthState } = await import('../Utils/index.js')
    ...
    try {
        ...
        if (err?.appeal_token) {
            ...
            return build('banned', true, false, {
                ...
                violationReason: VIOLATION_TYPES[err.violation_type] || 'Unknown',
                canAppeal: err.in_app_ban_appeal === 1,
                ...
                appealStatus: banDetails?.status || null,
