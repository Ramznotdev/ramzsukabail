...
class NexusHandler {
    ...
    parseTime(val, def) {
        let t = typeof val === 'string' ? parseInt(val) : (val || def)
        if (t > 9999999999) { // If milliseconds, convert to seconds
            t = Math.floor(t / 1000)
        }
        return t
    }
    ...
    async handleGroupStory(content, jid, quoted) {
        ...
        const msg = await this.genFromContent(jid, {
            groupStatusMessageV2: proto.Message.FutureProofMessage.create({ message: mediaContent })
        }, { userJid: jid })
