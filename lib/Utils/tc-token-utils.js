...
import { migrateIndexKey } from '../Utils/index.js';
...
export async function readTcTokenIndex(keys) {
    ...
    return Object.keys(batch).filter(j => j && j !== 'index');
}
...
export async function buildMergedTcTokenIndexWrite(keys, addedJids) {
    const batch = await migrateIndexKey(keys, 'tctoken');
    const merged = { ...batch };
    for (const jid of addedJids) { if (jid && jid !== 'index') merged[jid] = merged[jid] || true; }
    return { index: merged };
}
