...
export const generateLinkPreviewIfRequired = async (text, getUrlInfo, logger) => {
    ...
    if (!getUrlInfo) {
        console.log('[LinkPreview] Skipping preview: getUrlInfo function is undefined.');
        return
    }
    if (!url) {
        console.log('[LinkPreview] Skipping preview: No URL found in text.');
        return
    }
    console.log('[LinkPreview] Attempting to generate preview for URL:', url);
    try {
        const result = await getUrlInfo(url)
        if (!result) {
            console.log('[LinkPreview] getUrlInfo returned undefined/null for URL:', url);
        } else {
            console.log('[LinkPreview] Preview generated successfully for URL:', url);
        }
        return result
    }
    catch (e) {
        console.error('[LinkPreview] Error generating preview for URL:', url, e);
        logger?.warn({ trace: e.stack }, 'url generation failed')
    }
}
...
const handleTextMessage = async (message, options) => {
    ...
    console.log('[handleTextMessage] Starting. Initial message.linkPreview is:', urlInfo ? 'Defined' : 'Undefined');
    
    if (typeof urlInfo === 'undefined') {
        console.log('[handleTextMessage] message.linkPreview is undefined. Attempting to generate link preview using generateLinkPreviewIfRequired...');
        urlInfo = await generateLinkPreviewIfRequired(message.text, options.getUrlInfo, options.logger)
    } else {
        console.log('[handleTextMessage] Using pre-provided message.linkPreview.');
    }
    
    console.log('[handleTextMessage] urlInfo retrieved:', urlInfo ? 'Yes (defined)' : 'No (undefined)');
    if (urlInfo) {
        console.log('[handleTextMessage] Mapping urlInfo details to extContent:');
        console.log('  - Title:', urlInfo.title);
        console.log('  - Description:', urlInfo.description);
        console.log('  - Canonical URL:', urlInfo['matched-text']);
        console.log('  - jpegThumbnail:', urlInfo.jpegThumbnail ? `Present (${urlInfo.jpegThumbnail.length || Buffer.byteLength(urlInfo.jpegThumbnail)} bytes)` : '❌ Missing');
        ...
        console.log('  - highQualityThumbnail (img):', img ? '✅ Present' : '❌ Not present');
        if (img) {
            console.log('  - Mapping highQualityThumbnail to extContent properties:');
            console.log('    - directPath:', img.directPath);
            console.log('    - mediaKey:', img.mediaKey ? 'Present' : 'Missing');
            console.log('    - width x height:', img.width, 'x', img.height);
            Object.assign(extContent, { thumbnailDirectPath: img.directPath, mediaKey: img.mediaKey, mediaKeyTimestamp: img.mediaKeyTimestamp, thumbnailWidth: img.width, thumbnailHeight: img.height, thumbnailSha256: img.fileSha256, thumbnailEncSha256: img.fileEncSha256 })
        }
    } else {
        console.log('[handleTextMessage] No urlInfo was found or generated. Thumbnail will not be attached.');
    }
    ...
    console.log('[handleTextMessage] Final extContent returning keys:', Object.keys(extContent));
