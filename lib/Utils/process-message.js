...
async function storeTcTokensFromHistorySync(chats, signalRepository, keyStore, logger) {
    ...
    for (const chat of chats) {
        ...
        candidates.push({ rawJid: jid, storageJid, token: Buffer.from(chat.tcToken), ts, senderTs: chat.tcTokenSenderTimestamp ? toNumber(chat.tcTokenSenderTimestamp) : undefined })
    }
    if (!candidates.length) return
    const allStorageJids = [...new Set(candidates.flatMap(c => c.rawJid !== c.storageJid ? [c.storageJid, c.rawJid] : [c.storageJid]))]
    const existing = await keyStore.get('tctoken', allStorageJids)
    const entries = {}
    for (const c of candidates) {
        for (const key of (c.rawJid !== c.storageJid ? [c.storageJid, c.rawJid] : [c.storageJid])) {
            const existingEntry = existing[key]
            const existingTs = existingEntry?.timestamp ? Number(existingEntry.timestamp) : 0
            if (existingTs > 0 && existingTs >= c.ts) continue
            entries[key] = { ...existingEntry, token: c.token, timestamp: String(c.ts), ...(c.senderTs !== undefined ? { senderTimestamp: c.senderTs } : {}) }
        }
    }
