...
const extractVideoThumb = async (path, destPath, time, size) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        ffmpeg.on('close', code => code === 0 ? resolve() : reject(new Error(`FFmpeg thumbnail generation exited with code ${code}`)));
...
const convertToOpusBuffer = async (buffer, logger) => {
    const ffmpegPath = await getFfmpegPath();
    // Write to temp file so ffmpeg can seek (required for m4a/mp4 containers
    // that store the moov atom at the end of the file)
    ...
    try {
        return await new Promise((resolve, reject) => {
            ...
            ffmpeg.on('close', code => code === 0
                ...
                : reject(new Error(`FFmpeg Opus conversion exited with code ${code}`)));
            ffmpeg.on('error', reject);
        });
    } finally {
...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        // Dont Force-refresh aith upfront, it result to rate-overlimit on message with mutiple medias
